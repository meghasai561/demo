...
def authenticate():
    ...
    print("Before websocket connection")
    ...
    print("After websocket connection")
    ...
    if session['status']:
        ...
        logging.error(f"Authentication failed: {session.get('message', 'Unknown error')}")
        return False
...
def on_open(ws):
    ...
    print("Websocket Object is",ws)
    ws.subscribe(correlation_id, mode, token_list)
    # print("Subscribed to BankNifty ticks",subscribe_data)
